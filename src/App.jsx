...
function Broadcast({ user }) {
  ...
  const loadDraft = async () => {
    ...
    if (!error && data) {
      ...
      if (data.file_name) {
        ...
        if (VIDEO_EXTS.includes(ext)) setMediaType("video");
        else if (IMAGE_EXTS.includes(ext)) setMediaType("image");
      }
    }
    setDraftLoaded(true);
  };
  ...
  const saveDraft = async () => {
    ...
    const { error } = await supabase
      .from("posts")
      .upsert(
        { owner_id: user.id, file_name: fileName, file_url: uploadedUrl, caption, updated_at: new Date().toISOString() },
        { onConflict: "owner_id" }
      );

    setSaving(false);
    setSaveMessage(error ? "Save failed: " + error.message : "Saved — available on any device you log in from.");
  };
  ...
  const captureVideoFrames = (fileOrBlob, count = 3) => {
    return new Promise((resolve, reject) => {
      ...
      video.onloadedmetadata = () => {
        ...
        times = [d * 0.1, d * 0.5, d * 0.9].filter((t) => isFinite(t) && t >= 0);
      ...
      video.onerror = () => {
        URL.revokeObjectURL(objectUrl);
        reject(new Error("Could not read this video file in-browser."));
      };
    });
  };
  ...
  const analyzeContent = async () => {
    ...
    setContentAnalysis([]);
    try {
      let source = selectedFile;
      if (!source && fileUrl) {
        const res = await fetch(fileUrl);
        source = await res.blob();
      }
      if (!source) {
        setAnalysisError("Upload a video or image first.");
        setAnalyzing(false);
        return;
      }
      ...
      let images = [];
      if (mediaType === "video") {
        images = await captureVideoFrames(source, 3);
      } else if (mediaType === "image") {
        images = [await fileToBase64(source)];
      } else {
        setAnalysisError("Unrecognized file type — can't analyze this one.");
        setAnalyzing(false);
        return;
      }
      ...
      const functionsUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/analyze-content`;
      ...
      if (!res.ok || data.error) {
        ...
        setContentAnalysis(data.suggestions || []);
      }
    } catch (err) {
  ...
  const runAiReview = () => {
    ...
    const hasLocationWord = keywordLocation && caption.toLowerCase().includes(keywordLocation.toLowerCase());
    const hasServiceWord = keywordService && caption.toLowerCase().includes(keywordService.toLowerCase());
    if (keywordSuggestions.length > 0 && !(hasLocationWord && hasServiceWord)) {
      suggestions.push("You've generated search keyword suggestions but haven't added any to your caption yet — these help Google Business Profile and website search, separately from hashtags.");
    } else if (keywordSuggestions.length === 0 && !keywordService && !keywordLocation) {
      suggestions.push("No search keywords set — Google doesn't read hashtags, so if you want this to help you show up in Google search, add your service and city below to get keyword phrase suggestions.");
    }
  ...
  const generateKeywords = () => {
    const service = keywordService.trim();
    const location = keywordLocation.trim();
    if (!service && !location) return;
    const phrases = [];
    if (service && location) {
      phrases.push(`${service} in ${location}`);
      phrases.push(`${service} near ${location}`);
      phrases.push(`best ${service} ${location}`);
      phrases.push(`${location} ${service}`);
    }
    if (service) {
      phrases.push(`${service} near me`);
      phrases.push(`affordable ${service}`);
    }
    if (location) {
      phrases.push(`local ${location} business`);
    }
    setKeywordSuggestions([...new Set(phrases)]);
  };

  const addKeyword = (phrase) => {
    if (caption.toLowerCase().includes(phrase.toLowerCase())) return;
    setCaption((prev) => (prev.trim() ? `${prev.trim()} ${phrase}` : phrase));
  };
        ...
        {step === 2 && (
          <div className="bg-white border border-neutral-200 rounded-xl p-5">
            ...
            {draftLoaded && (
              <button
                onClick={saveDraft}
                disabled={saving || (!fileName && !caption)}
                className="w-full bg-blue-600 text-white text-sm font-medium py-2.5 rounded-lg mb-2 disabled:opacity-40"
              >
                {saving ? "Saving..." : "Save & sync to my account"}
              </button>
            )}
            {saveMessage && (
              <p className={`text-xs mb-4 ${saveMessage.startsWith("Save failed") || saveMessage.startsWith("Upload failed") ? "text-red-600" : "text-green-600"}`}>
                {saveMessage}
              </p>
            )}
            ...
            <div className="border border-neutral-200 rounded-lg p-3 mb-4">
              <p className="text-xs font-medium text-neutral-500 mb-1">Search keyword suggestions</p>
              <p className="text-xs text-neutral-400 mb-2">
                For Google Business Profile & your website — separate from hashtags, since Google doesn't read those.
              </p>
              <div className="flex gap-2 mb-2">
                <input
                  value={keywordService}
                  onChange={(e) => setKeywordService(e.target.value)}
                  placeholder="Service/product, e.g. dump truck sales"
                  className="flex-1 border border-neutral-200 rounded-lg px-3 py-2 text-sm"
                />
                <input
              ...
              <button
                ...
                disabled={!keywordService && !keywordLocation}
                className="w-full border border-neutral-300 text-neutral-700 text-sm font-medium px-3 py-2 rounded-lg disabled:opacity-40 mb-2"
              >
                Suggest keywords
              </button>
              {keywordSuggestions.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {keywordSuggestions.map((phrase) => {
                    const already = caption.toLowerCase().includes(phrase.toLowerCase());
                    return (
                      <button
                        key={phrase}
                        onClick={() => addKeyword(phrase)}
                        disabled={already}
                        className={`text-xs px-2.5 py-1 rounded-full border ${
                          already
                            ? "bg-neutral-100 text-neutral-400 border-neutral-200"
            ...
            <div className="border border-purple-200 bg-purple-50 rounded-lg p-3 mb-4">
              <p className="text-xs font-medium text-purple-900 mb-1">🔍 AI content analysis (looks at your actual video/image)</p>
              <p className="text-xs text-purple-700 mb-2">
                Unlike the format checklist below, this actually sends frames from your content to Claude for real analysis — a few cents at most per use.
              </p>
              <button
                onClick={analyzeContent}
                disabled={analyzing || (!fileName && !selectedFile && !fileUrl)}
                className="w-full bg-purple-600 text-white text-sm font-medium py-2.5 rounded-lg disabled:opacity-40"
              >
                {analyzing ? "Analyzing content..." : "Analyze content with AI"}
              </button>
              {analysisError && <p className="text-xs text-red-600 mt-2">{analysisError}</p>}
              {contentAnalysis.length > 0 && (
                <div className="mt-2 space-y-1.5">
                  {contentAnalysis.map((s, i) => (
                    <p key={i} className="text-xs text-purple-900">• {s}</p>
                  ))}
                </div>
              )}
            </div>
